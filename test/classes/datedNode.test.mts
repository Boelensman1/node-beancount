...
class TestDatedNodeClass extends DatedNode {
  type = 'transaction' as const

  static fromGenericParseResult(sourceFragment: string[]): TestDatedNodeClass {
    return new TestDatedNodeClass({ date: sourceFragment[0] })
  }

  static fromObject(inp: {
    date: string | Temporal.PlainDate
  }): TestDatedNodeClass {
    return new TestDatedNodeClass(inp)
  }
}
...
describe('DatedNode class', () => {
  describe('fromObject', () => {
    test('creates node with different valid dates', () => {
      ...
      dates.forEach((dateStr) => {
        const node = TestDatedNodeClass.fromObject({ date: dateStr })
    ...
    test('creates node with temporal date', () => {
      ...
      const node = TestDatedNodeClass.fromObject({
    ...
    test('throws error for malformed date string', () => {
      expect(() => {
        TestDatedNodeClass.fromObject({ date: 'not-a-date' })
      }).toThrow()
    })
    ...
    test('throws error for date of wrong type', () => {
      expect(() => {
        ...
        TestDatedNodeClass.fromObject({ date: new Date() })
      }).toThrow(
    ...
    test('throws error for invalid date format - no leading zeros', () => {
      expect(() => {
        TestDatedNodeClass.fromObject({ date: '2025-6-15' })
      }).toThrow()
    })
  })
  ...
  describe('type property', () => {
    test('has type property defined by subclass', () => {
      const node = TestDatedNodeClass.fromObject({ date: '2025-01-01' })
  ...
  describe('fromJSON', () => {
    test('creates node from JSON data', () => {
      ...
      const node = TestDatedNodeClass.fromJSON(JSON.stringify(jsonData))
    ...
    test('handles roundtrip serialization', () => {
      const original = TestDatedNodeClass.fromObject({ date: '2024-06-20' })
      const restored = TestDatedNodeClass.fromJSON(JSON.stringify(original))
